...
class Vec3fix : public Vec3 {
public:
	float spacingTest = 0.0;
	Vec3fix& operator =(const Vec3& vOther) {
...
struct Ray_t {
	Vec3fix  m_Start;  // starting point, centered within the extents
	Vec3fix  m_Delta;  // direction + length of the ray
	Vec3fix  m_StartOffset; // Add this to m_Start to Get the actual ray start
	Vec3fix  m_Extents;     // Describes an axis aligned box extruded along a ray
